  ...
  const filteredData = users
    .filter((user) => {
      ...
      const matchAccount =
        ...
        userEmail.includes(appliedFilters.account.toLowerCase());
      const matchStatus =
        appliedFilters.status === "all" ||
        user.status === Number(appliedFilters.status);
    ...
    if (r === "admin" || r === "staff" || r === "employer") {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-blue-100 text-blue-700 text-xs font-bold">
          <UserCog size={12} /> Nhân viên
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-gray-100 text-gray-700 text-xs font-bold">
    ...
    <div className="relative min-h-[80vh] font-sans pb-10">
              ...
              <form onSubmit={handleSaveUser} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 text-gray-900 rounded-lg px-4 py-2.5 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-800 mb-2">
                  ...
                  <div className="md:col-span-2 pt-4 border-t border-gray-50">
                    <label className="flex items-center gap-2 text-xs font-black text-blue-600 uppercase mb-4 tracking-widest">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full border border-gray-300 text-gray-900 rounded-lg px-3 py-2 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full border border-gray-300 text-gray-900 rounded-lg px-4 py-2.5 pl-12 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none font-bold"
                        />
                ...
                <div className="pt-8 flex justify-end gap-4 border-t border-gray-100 mt-8">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="px-6 py-2.5 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md font-semibold transition flex items-center gap-2"
                  >
                    <ArrowLeft size={18} /> Quay về
                  </button>
                  <button
                    type="submit"
                    className="px-8 py-2.5 text-white bg-blue-600 hover:bg-blue-700 rounded-md font-semibold transition flex items-center gap-2"
                  >
                    {modalMode === "add" ? (
                      <Plus size={18} />
                    ) : (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6 flex justify-between items-center">
        ...
        <button
          ...
          className="bg-blue-600 text-white px-5 py-2.5 rounded-lg shadow-sm hover:bg-blue-700 transition flex items-center gap-2 font-medium"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          ...
          <div>
            ...
            <select
              ...
              className="w-full border-2 border-gray-300 text-gray-900 rounded-lg px-4 py-2.5 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none shadow-sm transition-all bg-white font-bold"
            >
              <option value="customer">Khách hàng</option>
              <option value="staff">Nhân viên</option>
            </select>
          ...
          <div>
            <button
              ...
              className="bg-[#22C55E] text-white px-5 py-2.5 rounded-lg shadow-sm hover:bg-[#16A34A] transition flex items-center justify-center gap-2 font-black w-full md:w-auto"
            >
            ...
            <tbody className="divide-y divide-gray-200 text-sm">
              {!isLoading &&
                paginatedUsers.map((user, index) => (
                  <tr
                    key={user.id}
                    className={`hover:bg-blue-50/50 transition-colors ${user.resetRequested ? "bg-yellow-50" : ""}`}
                  >
                    ...
                    <td className="px-4 py-4 font-medium">
                      {renderRoleBadge(user.role || appliedFilters.role)}
                    </td>
                    {(appliedFilters.role === "staff" ||
                      appliedFilters.role === "admin") && (
                    ...
                    <td className="px-4 py-4 font-medium">
                      {user.resetRequested ? (
                        <span className="bg-yellow-100 text-yellow-800 px-2.5 py-1 rounded font-medium text-xs border border-yellow-300 inline-flex items-center gap-1">
                          Yêu cầu
                        </span>
                      ) : (
                        <span className="bg-gray-100 text-gray-500 px-2.5 py-1 rounded font-medium text-xs border border-gray-300 inline-flex items-center gap-1">
                          Không
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-4">
                      {user.status === 1 ? (
                        <span className="bg-green-50 text-green-800 px-2.5 py-1 rounded font-medium text-xs border border-green-300 inline-flex items-center gap-1">
                          <CheckCircle2 size={12} /> Hoạt động
                        </span>
                      ) : (
                        <span className="bg-red-50 text-red-700 px-2.5 py-1 rounded font-medium text-xs border border-red-300 inline-flex items-center gap-1">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex justify-center gap-2">
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-white font-medium text-xs transition ${user.status === 1 ? "bg-red-500 hover:bg-red-600" : "bg-gray-600 hover:bg-gray-700"}`}
                        >
